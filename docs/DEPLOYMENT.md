...
私有账户流和 `orders-algo` 流都必须连接并认证后才算实时交易状态就绪。
短暂重连不会立即告警；持续未就绪达到 `PRIVATE_STREAM_ALERT_GRACE_SECONDS`
（默认 30 秒，可设为 0 至 3600 秒）才尝试发送一次 PushPlus 告警，恢复后
尝试发送一次恢复通知。去重范围是当前进程的一次持续故障，重启重新计时；
发送失败留审计记录，不因每次检查或开仓重试反复发送。未配置 PushPlus 不发送通知。

该等待时间只影响告警，不延迟开仓拦截。缺少任一路状态、认证失败或断线时，
网页、内置策略和 TradingView 的非 Dry Run 新开仓都受同一执行器拦截；
预检前、预检后和杠杆准备后再次核验。Dry Run 和平仓不受新增的推送闸门限制，
但原有急停、执行开关、持仓与原生保护校验仍然有效。若账户数据等原有条件
仍满足而仅保护单通道不就绪，网页显示“仅可平仓”，不会将平仓按钮一起误锁。
认证就绪不代表订单已成交或已完成真实 OKX 验收。
...
控制台“风控与连接 / 账户连接检测”提供管理员专用的模拟盘只读检测。
点击后立即返回接收状态，后台在 45 秒预算内检查两路私有 WebSocket 认证和
余额、持仓、账户模式、当前/历史委托、成交、保护委托七项 REST 读取；
关闭检测连接后才发布通过结果。进度通过私有 SSE 同步，重复发起返回 `409`。
检测不使用或中断交易引擎原有推送连接，也不创建订单、不解除急停、不更改执行开关。

检测报告只保留连接状态和记录数量，不包含余额金额、订单内容、密钥或代理地址；
管理员失效或浏览器推送断开时清除页面结果。结果只在当前 API 进程内保留，
重启后重新检测；“只读连接通过”不代表订单成交或保护触发验收通过。
接口为经过管理员鉴权的 `POST /api/v1/account/connection-check`（发起）
和 `GET /api/v1/account/connection-check`（读取），私有推送事件为 `connection_check`。
凭据仍在服务器受保护的 `.env` 中管理，网页不提供密钥录入或回显。

配置 OKX Demo 私有凭据并更新 API 容器后，也可用下面的命令在运行中的容器内
...
Web 容器为静态资源返回 `Cache-Control: no-cache`，浏览器需通过 ETag 等条件请求
重新核对资源。页面脚本和样式的 `cache=validate-v1` 是迁移旧启发式缓存的一次性标记，
不是每个提交都要手动修改的发布版本号；后续更新依赖服务器重新校验。
发布验收仍须比对公网脚本摘要并在已打开的浏览器中刷新检查，不能只测新会话。
